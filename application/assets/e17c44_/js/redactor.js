...
(function($)
{
	...
	Redactor.fn = $.Redactor.prototype = {
		...
		pasteClean: function(html)
		{
			...
			html = html.replace(/<img alt="1" src="webkit-fake-url\:\/\/(.*?)"(.*?)>/gi, '');
			...
			if (this.browser('mozilla'))
			{
				...
				{
					var matches = html.match(/<img alt="2" src="data:image(.*?)"(.*?)>/gi);
					if (matches !== null)
					{
		...
		pasteClipboardUpload: function(e)
		{
			...
			if (this.opts.clipboardUpload)
			{
				...
				$.proxy(function(data)
				{
					...
					var html = '<img    alt="123"  src="' + json.filelink + '" id="clipboard-image-marker" />';
			...
			{
	        	this.insertHtml('<img  alt="123" src="' + result + '" />');
        	}
		},
				...
				if (this.opts.imageGetJson)
				{
					$.getJSON(this.opts.imageGetJson, $.proxy(function(data)
					{
						...
						$.each(data, $.proxy(function(key, val)
						{
							...
							var img = $('<img alt="" src="' + val.thumb + '" class="redactorfolder redactorfolder' + folderkey + '" rel="' + val.image + '" title="' + thumbtitle + '" />');
		...
		{
			var img = '<img alt="" id="image-marker" src="' + $(e.target).attr('rel') + '" alt="' + $(e.target).attr('title') + '" />';
